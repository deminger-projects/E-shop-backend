...
function load_json_files() {
    return __awaiter(this, void 0, void 0, function* () {
        yield Promise.all([
            ...
            //orders
            (0, write_json_1.default)(["SELECT orders.id, orders.name, orders.surname, orders.email, orders.adress, orders.phone, orders.postcode, orders.add_date, orders.status FROM orders;", "SELECT order_products.id, order_products.product_id, order_products.size, order_products.amount, products.name, products.price, products.discount, products.collection_id, products.description FROM order_products JOIN products ON products.id = product_id WHERE order_id = ? ;"], "../client/src/data/orders.json"),
            //refunds
            (0, write_json_1.default)(["SELECT refunds.id, orders.name, orders.surname, orders.email, orders.adress, orders.phone, orders.postcode, orders.add_date, refunds.status FROM refunds JOIN orders on refunds.order_id = orders.id WHERE refunds.status = 'Active' OR refunds.status = 'Accepted' ;", "SELECT refund_products.product_id, refunds.id, refund_products.amount, refund_products.size, order_products.prize, products.name, refund_reasons.reason FROM refund_products JOIN refunds ON refunds.id = refund_products.refund_id JOIN orders ON orders.id = refunds.order_id JOIN order_products ON order_products.order_id = orders.id JOIN products ON products.id = refund_products.product_id JOIN refund_reasons ON refund_products.reason_id = refund_reasons.id WHERE refunds.id = ? GROUP BY refund_products.product_id, refund_products.amount, refund_products.size, order_products.prize, products.name;"], "../client/src/data/refunds.json"),
            //refund reasons
