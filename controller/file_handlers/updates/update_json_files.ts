...
export default async function update_json_files(id?: number){
    ...
    if(id){
        ...
        //user data
        write_json(["SELECT users.id, users.username, users.email, users.password FROM users WHERE users.id = " + id + " ;", "SELECT user_data.id, user_data.user_id, user_data.name, user_data.surname, user_data.phone, user_data.adress, user_data.city, user_data.postcode, user_data.status, users.email FROM user_data JOIN users ON users.id = user_data.user_id WHERE user_id = " + id + " AND status = 'Active';"], "../client/src/data/user_data.json"),
           
        //user orders
        write_json(["SELECT orders.id, orders.name, orders.surname, orders.email, orders.adress, orders.phone, orders.postcode, orders.add_date, orders.status, (SELECT count(refunds.id) FROM refunds WHERE orders.id = refunds.order_id AND refunds.status = 'Active') as refund_count FROM orders WHERE orders.user_id = " + id + " ;", "SELECT order_products.id, order_products.product_id, products.name, order_products.size, order_products.amount, order_products.prize FROM order_products JOIN products on order_products.product_id = products.id WHERE order_id = $"], "../client/src/data/user_orders.json"),
        
        //login_data
        write_json(["SELECT users.id, users.username, users.password, users.login_status from users WHERE users.id = " + id + " ;"], "../client/src/data/login_data.json"),
    
        //user_refunds
        write_json(["SELECT refunds.id, orders.name, orders.surname, orders.email, orders.adress, orders.phone, orders.postcode, orders.add_date, refunds.status FROM refunds JOIN orders on refunds.order_id = orders.id WHERE refunds.status = 'Active' OR refunds.status = 'Accepted' AND orders.user_id = " + id + " ;", "SELECT refund_products.product_id, refunds.id, refund_products.amount, refund_products.size, order_products.prize, products.name, refund_reasons.reason FROM refund_products JOIN refunds ON refunds.id = refund_products.refund_id JOIN orders ON orders.id = refunds.order_id JOIN order_products ON order_products.order_id = orders.id JOIN products ON products.id = refund_products.product_id JOIN refund_reasons ON refund_products.reason_id = refund_reasons.id WHERE refunds.id = $ GROUP BY refund_products.product_id, refund_products.amount, refund_products.size, order_products.prize, products.name;"], "../client/src/data/user_refunds.json"),
