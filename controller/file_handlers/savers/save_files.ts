...
export default async function save_files(path: string, files: FileArray){
    ...
    fs.mkdir(path + "/temp", (err) => {   // creates folder
      if(err){
       ...
       if(multiple_files){
          for (let index = 0; index < multiple_files.length; index++) {
              multiple_files[index].mv(path + "/temp/" + multiple_files[index].name, (err) => { //save files in case of multiple files
                if(err){
          ...
          single_file.mv(path + "/temp/" + single_file.name, (err) => { //save files in case of single file
            if(err){
              console.log("🚀 ~ file: save_files.ts:13 ~ file.mv ~ err:", err.message)
            }else{
